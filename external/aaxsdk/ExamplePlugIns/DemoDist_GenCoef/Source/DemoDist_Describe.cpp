/*================================================================================================*/
/*
 *	Copyright 2009-2015, 2017, 2023 Avid Technology, Inc.
 *	All rights reserved.
 *	
...
static const AAX_CEffectID kEffectID_DemoDist = "com.avid.aax.sdk.demodist.gc";
...
static void DescribeAlgComponent (AAX_IComponentDescriptor& outDesc)
{
	...
	switch( StemFormat )
	{
		...
		case AAX_eStemFormat_Stereo:
		{
			...
			// Resource requirements
#ifndef AAX_TI_BINARY_IN_DEVELOPMENT // Define this macro when using a debug TI DLL to allocate only 1 instance per chip
			err = properties->AddProperty ( AAX_eProperty_TI_InstanceCycleCount, 1250 );
			err = properties->AddProperty ( AAX_eProperty_TI_SharedCycleCount, 252 );
#endif
...
static AAX_Result DescribeEffect(AAX_IEffectDescriptor& outDescriptor)
{
	...
	// Effect identifiers
	//
	err = outDescriptor.AddName ( "Demo Dist (GenCoef)" );
...
AAX_Result GetEffectDescriptions ( AAX_ICollection * outCollection )
{
	...
	err = outCollection->AddPackageName( "DemoDist (GenCoef) AAX SDK Example Plug-In" );
	err = outCollection->AddPackageName( "DemoDist (GenCoef) Plug-In" );
	err = outCollection->AddPackageName( "DemoDist (GenCoef)" );
	err = outCollection->AddPackageName( "DemoDist GC" );
