...
{
	constexpr uint32_t oneCharMask = 0x000000FF;
	constexpr int16_t oneCharNumBits = 8;
	constexpr auto unknownChar = "(?)"; // for current usage, a raw string here is slightly more efficient than a std::string
...
std::string AAX::AsStringPropertyValue(AAX_EProperty inProperty, AAX_CPropertyValue inPropertyValue)
{
		...
		AAX_eProperty_Constraint_Location == inProperty)
	{
		...
		constexpr int32_t maxNumBitsToShow = 8; // Currently there are no bitfield properties with more than 8 possible flags
